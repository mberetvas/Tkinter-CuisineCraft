...
pytesseract.pytesseract.tesseract_cmd = os.getenv("TESSERACT_CMD", "") # Default to empty string if not found in .env
...
class CuisineCraftModernGUI:
    def __init__(self, root):
        ...
        try:
            # Only check if Tesseract command is set
            if pytesseract.pytesseract.tesseract_cmd:
                pytesseract.get_tesseract_version()
            else:
                raise pytesseract.TesseractNotFoundError("Tesseract command not configured in .env")
        except pytesseract.TesseractNotFoundError:
            messagebox.showerror(
                "Tesseract Not Found",
                "Tesseract is not installed or not in your PATH. "
                "Please install it to use the receipt scanning feature."
            )
            # Optionally, disable the receipt tab
            # self.notebook.tab(self.tab_receipts, state="disabled")
    ...
    def setup_notebook(self):
        ...
        self.tab_week_menu = ttk.Frame(self.notebook, style='Modern.TFrame') # Random generator
        self.tab_manual_week_menu = ttk.Frame(self.notebook, style='Modern.TFrame') # Manual selection
        ...
        self.notebook.add(self.tab_week_menu, text='🎲 Auto Menu') # Renamed for clarity
    ...
    def setup_recipe_list_tab(self):
        ...
        refresh_btn = ttk.Button(button_frame, text="🔄 Refresh List", 
                               style='Modern.TButton', command=self.refresh_recipe_list)
        refresh_btn.pack(side='left', padx=(0, 8))
        ToolTip(refresh_btn, "Refresh the recipe list (Ctrl+R)")
        
        search_btn = ttk.Button(button_frame, text="🔍 Search", 
                              style='Secondary.TButton', command=self.search_recipes)
    ...
    def setup_week_menu_tab(self):
        ...
        generate_btn = ttk.Button(controls_frame, text="🎲 Generate Week Menu", 
                                style='Modern.TButton', command=self.generate_week_menu)
        ...
        export_btn = ttk.Button(controls_frame, text="📤 Export Menu",
                              style='Secondary.TButton', command=self.export_week_menu)
        ...
        remove_selected_btn = ttk.Button(controls_frame, text="🗑️ Remove Selected",
                                       style='Danger.TButton', command=self.remove_selected_menu_items)
    ...
    def setup_recipe_tab(self):
        ...
        save_btn = ttk.Button(button_frame, text="💾 Save Recipe", 
                            style='Modern.TButton', command=self.save_recipe)
        save_btn.pack(side='left', padx=(0, 8))
        
        clear_btn = ttk.Button(button_frame, text="🗑️ Clear Form", 
                             style='Secondary.TButton', command=self.clear_recipe_form)
    ...
    def setup_ingredients_tab(self):
        ...
        add_btn = ttk.Button(controls_frame, text="➕ Add Ingredient",
                           style='Modern.TButton', command=self.add_ingredient_entry)
        add_btn.pack(side='left', padx=(0, 8))

        remove_btn = ttk.Button(controls_frame, text="➖ Remove Ingredient",
                              style='Secondary.TButton', command=self.remove_ingredient_entry)
        ...
        save_btn = ttk.Button(button_frame, text="💾 Save Ingredients", 
                            style='Modern.TButton', command=self.save_ingredients)
        save_btn.pack(side='left', padx=(0, 8))
        
        clear_btn = ttk.Button(button_frame, text="🗑️ Clear Form", 
                             style='Secondary.TButton', command=self.clear_ingredients_form)
        clear_btn.pack(side='left')
    ...
    def setup_receipts_tab(self):
        ...
        upload_btn = ttk.Button(controls_frame, text="📄 Upload Receipt",
                                style='Modern.TButton', command=self.upload_receipt)
        ...
        save_btn = ttk.Button(controls_frame, text="💾 Save Items",
                              style='Secondary.TButton', command=self.save_receipt_items)
        save_btn.pack(side='left')
        ToolTip(save_btn, "Save the processed items to the database")

        # Add a label for processing feedback
        ...
        # Treeview for displaying receipt items
...
• Keyboard shortcuts
    ...
    def on_manual_menu_search_change(self, event=None):
        ...
        self._manual_search_after_id = self.root.after(300, self.refresh_manual_menu_recipe_list)

    def refresh_manual_menu_recipe_list(self):
        """Refresh recipe list for manual week menu with modern loading indicator"""
        self.status_bar.set_status("Refreshing manual menu recipe list...", show_progress=True)
        ...
        try:
            with DatabaseHandler() as db:
                ...
                if df.empty:
                    ...
                    self.status_bar.set_status(f"Loaded {len(df)} recipes for manual menu")
                    
        except Exception as e:
            logger.error(f"Failed to refresh manual menu recipe list: {str(e)}")
            self.status_bar.set_status(f"Error loading recipes for manual menu: {str(e)}")
            messagebox.showerror(
                "Error",
                f"Failed to refresh manual menu recipe list: {str(e)}"
            )
        finally:
            self.status_bar.set_status("Ready")
    ...
    def refresh_recipe_list(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                f"Failed to refresh recipe list: {str(e)}"
            )
        finally:
            self.status_bar.set_status("Ready")
    ...
    def populate_manual_menu_combos(self):
        ...
        try:
            with DatabaseHandler() as db:
                ...
                self.all_recipes_for_manual_menu = {row['ID']: row['recept_naam'] for _, row in df.iterrows()}
                recipe_list_display = [f"{row['ID']} - {row['recept_naam']}" for _, row in df.iterrows()]
                ...
                for day in self.days_of_week:
                    ...
                    if current_recipe_id and current_recipe_id in self.all_recipes_for_manual_menu:
                        combo.set(f"{current_recipe_id} - {self.all_recipes_for_manual_menu[current_recipe_id]}")
                    else:
                        combo.set("Select a recipe") # Reset if recipe not found or no selection
        ...
        try:
            with DatabaseHandler() as db:
                ...
                for i, (ingredient, amount, unit) in enumerate(results):
                    price, shop = self.find_ingredient_price(ingredient, receipt_items_df)
            ...
            self.status_bar.set_status(f"Error updating manual menu ingredients: {str(e)}")
            messagebox.showerror(
                "Error",
                f"Failed to update manual menu ingredients list: {str(e)}"
            )
    ...
    def find_ingredient_price(self, ingredient_name: str, receipt_items_df: pd.DataFrame) -> (Optional[float], Optional[str]):
    ...
    def export_week_menu(self):
        ...
        try:
            meal_names = [
                self.week_menu_listbox.get(i).split(') ', 1)[-1] # Extract only the meal name
    ...
    def populate_recipe_combo(self):
        ...
        try:
            with DatabaseHandler() as db:
                df = db.get_recipes_for_combo()
                recipe_list = [f"{row['ID']} - {row['recept_naam']}" for _, row in df.iterrows()]
    ...
    def setup_keyboard_shortcuts(self):
        ...
        self.root.bind('<Control-n>', lambda e: self.notebook.select(self.tab_add_recipe))
    ...
    def run(self):
        ...
        try:
            ...
            # Start the main loop
            self.status_bar.set_status("CuisineCraft ready! Use Ctrl+N for new recipe, Ctrl+R to refresh")
            self.root.mainloop()
